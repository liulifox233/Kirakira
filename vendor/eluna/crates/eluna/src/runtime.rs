...
mod reverse_parity_tests {
    ...
    fn timeline_hold_markers_do_not_pollute_authored_ranges() {
        ...
        merge_timeline_variable_info(&variable, &mut out);
        let info = &out["pose"];
        // A timeline-only variable starts at the scalar zero default
        // (sub_1026FA30 parses the commands and does not use the first one as
        // an initial value; see the sibling test
        // `timeline_only_variable_does_not_take_first_key_as_initial_value`).
        // The hold marker's synthetic 0.0 still stays out of the authored
        // range, which is what this test is about: 3.0..5.0, two live frames.
