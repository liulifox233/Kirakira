...
fn kag_load_scenario(
    ...
    with_parser(
        ...
        |parser, vm, owner| {
            ...
            let loaded = parser.load_scenario_with(storage, &mut host);
            loaded.map_err(|error| host.script_exception_or(error))?;
            Ok(Variant::Void)
        },
    )
}
...
fn register_system_hooks(host: &mut KrkrHost, text: &str) {
    for line in text.lines() {
        // The reference's scenario reader skips a `;` line before it looks for
        // tags, so a declaration that is commented out is not in its hook table
        // either.
...
/// engine-driven scan, and treats an empty value as absent where this table
/// keeps it (`kag_decl_attr` returns `Some("")`).
...
fn kag_get_next_tag(
    ...
    with_parser(
        ...
        |parser, vm, owner| {
            ...
            let Some(tag) = parser
                .next_tag_with(&mut host)
                .map_err(|error| host.script_exception_or(error))?
            else {
                host.vm
                    .runtime_mut()
                    .host_mut()
...
struct TjsKagHost<'a, 'bc, 'rt> {
    ...
    /// `KagError::Host`, kept until the native method that drove the parser
    /// can hand it back to the VM.
    ///
    ...
    /// `base/ScriptMgnIntf.h:94`; see `handle_escaped_script_exception`).
    /// `KagError` can only carry the rendered text, so without this the
    /// handler would see a plain `Exception` where the game threw e.g. a
    /// `ConductorException` -- `e instanceof ConductorException` in
    /// `Initialize.tjs:18-36` would stop deciding.
...
impl<'a, 'bc, 'rt> TjsKagHost<'a, 'bc, 'rt> {
    ...
    /// The `TjsError` the VM should see for a parser failure: the original
    /// script exception when the parser's error came from a host callback,
    /// otherwise the plain host conversion. A park
    /// (`KagError::ResourcePending` / `KagError::HostSuspended`) never sets
    /// the stash, so those keep their own marker.
    fn script_exception_or(&mut self, error: KagError) -> TjsError {
    ...
    fn call_event(&mut self, name: &str, args: Vec<Variant>) -> krkr_kag::Result<Option<Variant>> {
        ...
        if matches!(member.map_err(kag_tjs_error)?, Variant::Void) {
            return Ok(None);
        }
        let value = self.vm.call_object_method(self.owner, name, args);
        match value {
...
impl KagHost for TjsKagHost<'_, '_, '_> {
    ...
    fn eval_bool(&mut self, expression: &str) -> krkr_kag::Result<bool> {
        self.ensure_not_suspended()?;
        let value = eval_expression(self.vm.runtime_mut(), self.owner, expression);
...
fn ensure_snapshot_storage_loaded(
    ...
    match parser.set_cur_storage(storage.to_string()) {
        ...
        Err(KagError::ScenarioNotLoaded { .. }) => {
            let mut host = TjsKagHost::new(vm, owner);
            let loaded = parser.load_scenario_with(storage.to_string(), &mut host);
            loaded.map_err(|error| host.script_exception_or(error))
        }
...
/// the failure itself (`TjsKagHost::script_error`) so the original exception
...
mod tests {
    ...
    /// A declaration the reference's scenario reader skips as a comment must
    /// not reach the host table either: the reader strips leading tabs when it
    /// splits a scenario into lines (`KAGParser.cpp:147-148`) and then skips
    /// any line whose first character is `;` (`:1094-1095`), which covers the
    /// `;;` declarations PARQUET (`custom.ks:40,41,43`) and GINKA (`:48`)
    /// comment out and 少女世界's single-`;` ones.
    #[test]
    fn register_system_hooks_skips_commented_out_declarations() {
        let mut engine = KrkrEngine::new(EngineConfig::default()).expect("engine");
        register_system_hooks(
            engine.host_mut(),
            // The first line is the tab-indented comment shape of PARQUET's
            // `custom.ks:21`; the rest are verbatim shipped declarations.
            "\t;;\t[addSysHook name=\"tabbed.comment\" call storage=\"custom.ks\" target=*logo]\n\
             ...
             ;;	[addSysScript name=\"title.from.soundmode\"    storage=\"custom.ks\" target=*title_extra_snd]\n",
        );

        let active = engine
            .host()
            .system_hook("first.logo")
            .expect("active hook");
        assert_eq!(active.storage.as_deref(), Some("custom.ks"));
        assert_eq!(active.target.as_deref(), Some("*logo"));
        assert!(active.call);

        assert!(engine.host().system_hook("tabbed.comment").is_none());
        assert!(engine.host().system_hook("scenemode.restore").is_none());
        assert!(engine.host().system_hook("exit.begin").is_none());
    ...
    fn register_system_hooks_reads_call_false_as_a_jump() {
        ...
        let call = engine.host().system_hook("probe.call").expect("call hook");
        let decoy = engine
            .host()
