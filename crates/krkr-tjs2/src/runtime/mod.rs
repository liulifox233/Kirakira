...
impl<H: TjsHost + 'static> Runtime<H> {
    ...
    ///
    /// A denied property is denied for *plain* stores only.  A store that
    /// carries `TJS_IGNOREPROP` (the regmember-style `&obj.prop = v`, VM_SPDS)
    /// skips the property object the way `tTJSCustomObject::PropSet` does
    /// (`tjsObject.cpp:1519-1541`) and copies the value into the member slot,
    /// so a script can still replace the member outright -- which is exactly
    /// how KAGEX swaps a layer's font (`&layer.font = hook`,
    /// `sysscn/prerenderfontex.tjs`).
