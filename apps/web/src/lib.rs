    ...
    pub fn tick(&mut self, timestamp_ms: f64) -> Result<js_sys::Object, wasm_bindgen::JsValue> {
        ...
        if !output.transitions.is_empty() {
            ...
            for transition in &output.transitions {
                ...
                js_sys::Reflect::set(
                    &transition_model,
                    &wasm_bindgen::JsValue::from_str("sourceDrawList"),
                    &source_draws,
                )?;
                let source_uploads = image_uploads_to_js(&transition.source_image_uploads)?;
                js_sys::Reflect::set(
                    &transition_model,
                    &wasm_bindgen::JsValue::from_str("sourceUploads"),
                    &source_uploads,
                )?;
